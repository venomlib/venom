...
export async function generateASCIIQR(
  input: string,
  opts: { small?: boolean } = {}
): Promise<string> {
  const BLACK = true;
  ...
  const QRCode =
    (await import('../../vendor/QRCode/index.js')).default ||
    (await import('../../vendor/QRCode/index.js'));
  const QRErrorCorrectLevel =
    (await import('../../vendor/QRCode/QRErrorCorrectLevel.js')).default ||
