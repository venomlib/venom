...
export class GroupLayer extends RetrieverLayer {
    ...
    async setGroupImage(groupId, path) {
        ...
        if (b64) {
            ...
            if (!mimeInfo || mimeInfo.includes('image')) {
                let _webb64_96 = await resizeImg(buff, { width: 96, height: 96 }), _webb64_640 = await resizeImg(buff, { width: 640, height: 640 });
    ...
    async getChatGroupNewMsg() {
        return await this.page.evaluate(async () => {
            // @ts-ignore
            return (await WAPI.getAllChats()).filter((chat) => chat.unreadCount > 0 && chat.kind === 'group');
        });
    }
    /**
    ...
    async getGroupAdmins(groupId) {
        ...
        const result = await this.page.evaluate((groupId) => WAPI.getGroupAdmins(groupId), groupId);
        if (result['erro'] == true) {
            throw result;
        }
        return result;
