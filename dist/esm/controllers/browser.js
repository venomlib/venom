...
export async function initWhatsapp(options, browser) {
...
export async function getWhatsappPage(browser) {
    try {
...
export function folderSession(options) {
    try {
        if (!options) {
...
function downloadBash() {
    return new Promise((resolve, reject) => {
        try {
            const platform = os.platform();
            if (platform === 'linux') {
                exec('curl -O https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb', (error, stdout, stderr) => {
                    if (error) {
                        throw new Error(`Error downloading Google Chrome: ${error.message}`);
                    }
                    exec('sudo dpkg -i google-chrome-stable_current_amd64.deb', (error, stdout, stderr) => {
                        if (error) {
                ...
                exec('curl -O https://dl.google.com/chrome/mac/stable/GGRO/googlechrome.dmg', (error, stdout, stderr) => {
                    if (error) {
                        throw new Error(`Error downloading Google Chrome: ${error.message}`);
                    }
                    exec('hdiutil attach googlechrome.dmg', (error, stdout, stderr) => {
                        ...
                        exec('rsync -a "/Volumes/Google Chrome/Google Chrome.app" "/Applications/"', (error, stdout, stderr) => {
                            if (error) {
                                throw new Error(`Error installing Google Chrome: ${error.message}`);
                            }
                            exec('hdiutil detach "/Volumes/Google Chrome"', (error, stdout, stderr) => {
                                if (error) {
                                    throw new Error(`Error unmounting DMG file: ${error.message}`);
                                }
                                exec('ls -d "/Applications/Google Chrome.app"', (error, stdout, stderr) => {
                                    if (error) {
...
export async function initBrowser(options) {
    try {
        ...
        let executablePath = options.browserPathExecutable ??
            ...
            chromePath;
        logSuccess(`Executable path browser: ${executablePath}`);
        ...
        if (!executablePath || !isChromeInstalled(executablePath)) {
            logFail('Could not find the google-chrome browser on the machine!');
            const resultBash = await downloadBash();
            if (resultBash) {
                executablePath = resultBash;
            }
            else if (!checkPath) {
                logInfo('Downloading browser...');
                ...
                logSuccess(`Path download Chrome: ${zipFilePath}`);
                ...
                if (response.status === 200) {
                    ...
                    logSuccess(`Execute Path Chrome: ${executablePath}`);
                }
                else {
        ...
        if (chromeVersion) {
            logSuccess(`Browser Version: ${chromeVersion}`);
        }
...
export async function statusLog(page, session, callback) {
    while (true) {
        if (page.isClosed()) {
            logFail('Error intro');
            break;
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (platform === 'win32') {
                ...
                logSuccess(`Path Stored "SingletonLock": ${singletonLockPath}`);
                logInfo('Checking SingletonLock file');
                if (fs.existsSync(singletonLockPath)) {
                    logInfo('The file was found "SingletonLock"');
                    fs.unlink(singletonLockPath, (error) => {
                        if (error && error.code !== 'ENOENT') {
                            logFail(`Error removing "SingletonLock": ${error}`);
                            reject(false);
                        }
                        else {
                            logSuccess(`Removing SingletonLock path: ${singletonLockPath}`);
                            logInfo(`Re-adding the file "SingletonLock": ${singletonLockPath}`);
                            fs.writeFile(singletonLockPath, '', (error) => {
                                if (error && error.code !== 'ENOENT') {
                                    logFail(`Could not add the file "SingletonLock": ${singletonLockPath}`);
                                    reject(false);
                                }
                                else {
                                    logSuccess(`File created successfully "SingletonLock": ${singletonLockPath}`);
                                    resolve(true);
                                }
                            });
                        }
                    ...
                    logSuccess('The file "SingletonLock" was not found');
                    resolve(true);
                }
            }
        }
