...
const controls_layer_js_1 = require("./layers/controls.layer.js");
const decrypt_js_1 = require("./helpers/decrypt.js");
...
async function checkFileExists(filePath) {
    try {
        await promises_1.default.access(filePath, promises_1.default.constants.F_OK); // F_OK checks existence
        return true;
    }
    catch (error) {
...
class Whatsapp extends controls_layer_js_1.ControlsLayer {
    browser;
    page;
    ...
    async initService() {
        try {
            ...
            if (await checkFileExists(path.join(getDirname(), '../../lib/wapi/', 'wapi.js'))) {
                js = await promises_1.default.readFile(path.join(getDirname(), '../../lib/wapi/', 'wapi.js'), 'utf-8');
            }
            else {
                js = await promises_1.default.readFile(path.join(process.cwd(), 'node_modules/venom-bot/dist/lib/wapi', 'wapi.js'), 'utf-8');
            }
            ...
            if (await checkFileExists(path.join(getDirname(), '../../lib/middleware/', 'middleware.js'))) {
                middleware_script = await promises_1.default.readFile(path.join(getDirname(), '../../lib/middleware', 'middleware.js'), 'utf-8');
            }
            else {
                middleware_script = await promises_1.default.readFile(path.join(process.cwd(), 'node_modules/venom-bot/dist/lib/middleware', 'middleware.js'), 'utf-8');
            }
    ...
    async decryptFile(message) {
        const options = (0, decrypt_js_1.makeOptions)(WAuserAgente_js_1.useragentOverride);
        message.clientUrl =
        ...
        try {
            while (haventGottenImageYet) {
                ...
                if (res.status == 200) {
                    ...
                    await (0, decrypt_js_1.timeout)(2000);
                }
            }
        }
        catch (error) {
        ...
        return (0, decrypt_js_1.magix)(buff, message.mediaKey, message.type, message.size);
    }
}
